# Same as p10_plot_meshes_side_by_side.jl but for the amplitude=30%
# case from scripts/p10_PEC_sphere_perturbation_amplitude_sweep.jl
# (realization=1 fixed, only amplitude scaled up to 0.30) -- the most
# extreme perturbation in that sweep, for which the DOF-aligned frozen
# Calderon preconditioner still converged in 13 iterations (vs 12 for
# a freshly-rebuilt preconditioner).
#
# Only exercises the `geo` target (mesh generation, no EFIE assembly),
# and amplitude=0.30/realization=1 was already computed by the sweep,
# so this should hit Makeitso's on-disk cache and render instantly.
...
reldisp = [(norm(vertsp[i]) - radius)/radius*100 for i in eachindex(vertsp) if norm(verts0[i]) >= 1e-8*radius]
println("actual radial displacement: min=", round(minimum(reldisp), digits=2), "%  max=", round(maximum(reldisp), digits=2),
    "%  mean=", round(sum(reldisp)/length(reldisp), digits=3), "%")
...
outfile = projectdir("data", "figures", "p10_mesh_side_by_side_amp30.html")
