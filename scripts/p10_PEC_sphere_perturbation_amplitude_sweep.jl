# Simple test case: how does the DOF-aligned frozen-preconditioner
# strategy behave as the perturbation amplitude grows well beyond the
# 3% used in the main N=20 campaign
# (scripts/p10_PEC_sphere_perturbation_robustness_v2.jl), up to 20-30%?
#
# realization is held FIXED (=1) across the whole sweep: since
# postproc/shape_perturbation.jl's coefficients are
# amplitude * decay(l) * Uniform(-1,1)-draw-from-seeded-rng, keeping
# the same realization/seed while only changing `amplitude` scales the
# SAME underlying random deformation pattern up and down, rather than
# drawing a new independent random shape each time -- so this isolates
# the effect of deformation SIZE on preconditioner transfer, holding
# its "direction" fixed.
#
# For each amplitude, also reports the ACTUAL achieved radial
# displacement range (the nominal `amplitude` parameter is not the same
# as the realized min/max displacement, since coefficients are random
# and l-dependent), plus a basic mesh-validity check (any triangle
# becoming degenerate/inverted), before trusting the solve results.
#
# Registers to data/sweep/p10_amplitude_sweep_summary.csv. Wrapped in
# try/catch per amplitude; robust to interruption (each amplitude's row
# appended immediately).
#
# Run directly: F5 in VS Code, or
# `julia scripts/p10_PEC_sphere_perturbation_amplitude_sweep.jl` from
# the project root.
...
amplitudes = [0.40, 0.50]
# NOTE: previously ran [0.03, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30] --
# those rows are already in summary_file. Only running the two new,
# more extreme amplitudes here to avoid re-appending duplicate rows
# for the already-completed (and disk-cached) points.

outdir = projectdir("data", "sweep")
mkpath(outdir)
summary_file = joinpath(outdir, "p10_amplitude_sweep_summary.csv")
...
println("="^70)
println("Perturbation AMPLITUDE sweep (DOF-aligned frozen strategy) -- started ", now())
...
for amp in amplitudes
    ...
    try
        ...
        # --- Actual achieved radial displacement + mesh validity check
...
println("\n" * "="^70)
println("Amplitude sweep complete -- ", now())
