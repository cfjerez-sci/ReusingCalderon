...
#
# Writes data/sweep/p22_kappa_mismatch_bidirectional.csv, consumed by
# kappa_mismatch/analyze_kappa_mismatch.py. Does NOT touch
# data/sweep/p10_kappa_mismatch_precond_summary.csv (Section 5.6).
#
# Run:
#   julia scripts/p22_kappa_mismatch_bidirectional.jl --smoke     # ~minutes, h=0.4
#   julia scripts/p22_kappa_mismatch_bidirectional.jl --detune    # the real run
#   julia scripts/p22_kappa_mismatch_bidirectional.jl --lowk --detune   # +3 octaves down
...
# --lowk: three further octaves downward, d = -4, -5, -6 relative to KAPPA0_A.
# Motivation: at kappa0 = 2 the downward penalty R went 1.00, 2.25, 3.00, 4.00
# at |d| = 0..3 (increments 1.25, 0.75, 1.00), and the kappa_precond = 4 row
# went 1.00, 4.78, 7.50, 8.62 (increments 3.78, 2.72, 1.12) -- decelerating,
# i.e. hinting at a CEILING on downward reuse, whereas the upward arm
# accelerates (1.00, 2.88, 6.48; increments 1.88, 3.60). These three octaves
# test whether the downward penalty really saturates.
# NOTE kappa*h reaches 0.003 here, so plain GMRES may hit the 1500-iteration
# cap; that is RECORDED, not discarded, and is not a failure of the experiment.
# Watch the true-residual column rather than the iteration count this far down:
# attainable accuracy is expected to degrade before Krylov convergence does.
KLOW      = [0.125, 0.0625, 0.03125]

if lowk
    append!(KSOLVE_A, KLOW)
    append!(KSOLVE_D, KLOW)
    sort!(KSOLVE_A); sort!(KSOLVE_D)
end
...
mkpath(outdir)
csvfile = joinpath(outdir,
    smoke ? "p22_kappa_mismatch_bidirectional_SMOKE.csv" :
    lowk  ? "p22_kappa_mismatch_bidirectional_LOWK.csv"  :
            "p22_kappa_mismatch_bidirectional.csv")
...
println("parts = $partsel   detune = $detune   smoke = $smoke   lowk = $lowk")
...
lowk   && println("Low-kappa extension: kappa = $KLOW  (d = -4, -5, -6)")
...
function record!(; part, κp, κs, strategy, u, ch, tsolve, Zxx, bx,
    ...
    # NB: @printf needs a single literal format string; a `*` concatenation
    # fails at macro-expansion time.
