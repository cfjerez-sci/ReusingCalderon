# Same campaign as scripts/p10_PEC_sphere_perturbation_robustness_v2.jl
# (N=20 independent random shape realizations, DOF-aligned frozen
# Calderon preconditioner vs. plain GMRES vs. fresh Calderon) but at
# amplitude=30% instead of the original 3% -- to get the same kind of
# statistical spread (boxplot) at the much larger, previously
# single-realization-only-tested 30% amplitude level.
#
# realization=1 at amplitude=0.30 was already computed by
# scripts/p10_PEC_sphere_perturbation_amplitude_sweep.jl, so that point
# should hit Makeitso's on-disk cache; realizations 2..20 all require a
# fresh discretization (~7-8 min assembly each) -- ~2.5 hours total.
#
# Registers to data/sweep/p10_perturbation_summary_v2_amp30.csv. Seeded
# (realization -> seed 1000+realization, same convention as the
# original v2 campaign) for full reproducibility; robust to
# interruption (each realization's row appended to disk immediately);
# wrapped in try/catch per realization.
#
# Run directly: F5 in VS Code, or
# `julia scripts/p10_PEC_sphere_perturbation_robustness_v2_amp30.jl`
...
κ          = 2.0
lmin, lmax = 2, 6
amplitude  = 0.30   # <-- 30%, vs. the original campaign's 3%

nominal_lmin, nominal_lmax, nominal_amplitude = 2, 6, 0.03

N = 20   # full campaign, matching the original v2 N=20

outdir = projectdir("data", "sweep")
mkpath(outdir)
summary_file = joinpath(outdir, "p10_perturbation_summary_v2_amp30.csv")
...
println("="^70)
println("PEC sphere shape-perturbation robustness study v2 @ amplitude=30% -- started ", now())
println("h=$h  kappa=$κ  realizations=$N  lmin=$lmin  lmax=$lmax  amplitude=$amplitude")
println("="^70)

# --- Nominal (unperturbed) discretization + spaces: built once (shared
# with the original 3% campaign -- same sentinel values, so this should
# hit disk cache).
...
println("\n" * "="^70)
println("Perturbation robustness study v2 @ amplitude=30% complete -- ", now())
