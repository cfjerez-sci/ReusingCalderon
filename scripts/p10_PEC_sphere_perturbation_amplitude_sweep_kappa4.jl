# Same as p10_PEC_sphere_perturbation_amplitude_sweep.jl (which used
# kappa=2.0) but at kappa=4.0, same h=0.1 and same realization=1 /
# amplitude grid -- to see whether the DOF-aligned frozen Calderon
# preconditioner's robustness to shape perturbation holds up at a
# higher wavenumber (more oscillatory kernel, more iterations needed
# in general).
#
# At h=0.1, kappa=4.0 gives wavelength lambda=2*pi/kappa=1.571, so
# ~16 points per wavelength -- still comfortably resolved (rule of
# thumb ~10 ppw), same mesh/dof as the kappa=2.0 campaign.
#
# Registers to data/sweep/p10_amplitude_sweep_summary_kappa4.csv
# (separate from the kappa=2.0 file). Wrapped in try/catch per
# amplitude; robust to interruption (each amplitude's row appended
# immediately).
#
# Run directly: F5 in VS Code, or
# `julia scripts/p10_PEC_sphere_perturbation_amplitude_sweep_kappa4.jl`
...
amplitudes = [0.50]
# NOTE: [0.03 .. 0.40] already completed and saved (8/9 rows) -- that
# run's Julia process was killed mid-way through the last (50%) point
# by an unrelated VS Code auto-update/restart. Only rerunning the
# missing point here to avoid duplicate rows.

outdir = projectdir("data", "sweep")
mkpath(outdir)
summary_file = joinpath(outdir, "p10_amplitude_sweep_summary_kappa4.csv")
...
println("="^70)
println("Perturbation AMPLITUDE sweep at kappa=4.0 (DOF-aligned frozen strategy) -- started ", now())
...
println("\n" * "="^70)
println("Amplitude sweep (kappa=4.0) complete -- ", now())
